#!/usr/bin/env node
// Обновляет промпт голосового агента dental (dental-ru).
// Запуск: node dasha-setup/update-dental-agent.mjs
...
const DENTAL_AGENT = '33b13492-2055-444b-b540-0f08d3304ce3';
...
const cur = await (await fetch(`${BASE}/api/v1/agents/${DENTAL_AGENT}`, { headers: H })).json();
const body = { ...cur, config: { ...cur.config, llmConfig: { ...cur.config.llmConfig, prompt: PROMPT } } };
delete body.id; delete body.createdAt; delete body.updatedAt;

const res = await fetch(`${BASE}/api/v1/agents/${DENTAL_AGENT}`, {
  method: 'PUT', headers: H, body: JSON.stringify(body),
});
console.log(res.ok ? `✅ Промпт обновлён (${DENTAL_AGENT})` : `❌ ${res.status}: ${await res.text()}`);
