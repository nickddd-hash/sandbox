    ...
    switch (e.name) {
      ...
      case 'add_order_item': {
        ...
        set((s) => {
          ...
          // Сигнатура позиции: без пунктуации/скобок, без стоп-слов, со стеммингом (4 буквы),
          // токены отсортированы. Так «заготовка для шаурмы курица», «Заготовка для шаурмы
          // (курица)» и «куриная заготовка для шаурмы» дают ОДНУ сигнатуру (модель часто
          // переписывает название при пересказе заказа → иначе плодятся дубли).
          const STOP = new Set(['для', 'из', 'с', 'и', 'в', 'на', 'по', 'от', 'до', 'шт', 'кг', 'б', 'к']);
          const sig = (n: string) =>
            n
              .toLowerCase()
              ...
              .sort()
              .join('|');
          // Повторный вызов той же позиции (по сигнатуре) = ОБНОВЛЕНИЕ количества/цены,
          // а не дубль. Покрывает и преждевременный qty:1, и перефразированное название.
          const itemSig = sig(item.name);
          const idx = itemSig ? s.order.findIndex((x) => sig(x.name) === itemSig) : -1;
          if (idx >= 0) {
