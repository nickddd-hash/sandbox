    ...
    switch (e.name) {
      ...
      case 'add_order_item': {
        ...
        set((s) => {
          ...
          // 1) Плейсхолдеры — НЕ реальная позиция. Модель добавляет «лаваш (уточнить вид)»,
          //    «шаурма — вариант?» пока уточняет спецификацию. Не заносим вовсе:
          //    настоящая позиция придёт следующим вызовом (см. правило НЕ ПРЕДЗАПОЛНЯЙ).
          if (/уточн|вариант|выбрать|\bвид\b|\?|не указан|не выбран/i.test(item.name)) return {};

          // 2) Дедуп для ЛЮБЫХ позиций. Токены названия: без пунктуации/скобок, без стоп- и
          //    шумовых слов, стем 4 буквы. Совпадение = одинаковый набор ИЛИ вложенность
          //    (токены одной ⊆ другой) при совместимой цене. Так «шаурма» / «шаурма курица» /
          //    «Заготовка для шаурмы (курица)» и «лаваш» / «лаваш тонкий» — одна позиция
          //    (уточнение спецификации), а «филе грудки» vs «филе бедра» (токены и цена
          //    разные) — две. Покрывает и преждевременный qty:1, и перефразирование при
          //    пересказе заказа.
          const NOISE = new Set(['для', 'из', 'с', 'и', 'в', 'на', 'по', 'от', 'до', 'шт', 'кг', 'б', 'к', 'тип', 'нужн', 'ваш', 'како', 'наш']);
          const toks = (n: string) =>
            new Set(
              n
                .toLowerCase()
          ...
          const B = toks(item.name);
          const isSub = (A: Set<string>, C: Set<string>) => A.size > 0 && [...A].every((t) => C.has(t));
          const idx =
            B.size === 0
              ? -1
              : s.order.findIndex((x) => {
                  ...
                  if (!(isSub(A, B) || isSub(B, A))) return false;
                  // Сливаем только при совместимой цене (или если у одной стороны цены нет) —
                  // иначе «бедро» 265 и «филе бедра» 425 ошибочно схлопнулись бы в одну.
                  return item.price === x.price || !item.price || !x.price;
                });
          if (idx >= 0) {
            const order = s.order.slice();
            const x = order[idx];
            // Оставляем более конкретное название (больше токенов) и свежие qty/цену.
            const keepName = B.size >= toks(x.name).size ? item.name : x.name;
            order[idx] = { ...x, name: keepName, qty: item.qty, price: item.price || x.price };
            return { order };
          }
          let order = s.order;
